...
echo "<!-- DEBUG: Prediction from session: " . ($_SESSION['last_prediction'] ?? 'NOT SET') . " -->";
echo "<!-- DEBUG: GET upload: " . ($_GET['upload'] ?? 'NOT SET') . " -->";
...
if ($temp_file && file_exists($temp_file)) {
    ...
    // Set status based on prediction (check if it contains 'normal')
    $status = (strpos(strtolower($prediction), 'normal') !== false) ? 'normal' : 'abnormal';
...
elseif ($waveImg_id > 0) {
    ...
    // Use stored values if they exist, otherwise fallback to static array
    if (!empty($analysis['explanation'])) {
        $explanation = $analysis['explanation'];
    } else {
        $info = getAnomalyInfo($anomaly_type, $anomalyInfo);
        $explanation = $info ? $info['explanation'] : 'No explanation available.';
    }

    if (!empty($analysis['suggested_recommendation'])) {
        $suggested_recommendation = $analysis['suggested_recommendation'];
    } else {
        $info = getAnomalyInfo($anomaly_type, $anomalyInfo);
        $suggested_recommendation = $info ? $info['suggested_recommendation'] : 'No recommendation available.';
    }
}
...
if ($is_temp) {
    ...
    $display_status = (strpos(strtolower($prediction), 'normal') !== false) ? 'normal' : 'anomaly';
                ...
                <div class="result-header">
                    ...
                    <?php else: ?>
                        <span class="status-badge <?= $display_status ?>">
                            <i class="fas fa-<?= $display_status === 'anomaly' ? 'exclamation-triangle' : 'check-circle' ?>"></i>
                    ...
                    <div>
                        ...
                        <div class="combined-info-item">
                            <div class="info-label">
                                ...
                                <span style="color: <?php echo ($prediction !== 'normal' && $prediction !== null && strpos(strtolower($prediction), 'normal') === false) ? 'var(--danger)' : 'var(--success)'; ?>; font-weight: 700; margin-left: 5px;">
